...
export default function IntroScreen() {
 ...
 const RegistrationScreenValue = useMemo(()=>({screen,setScreen}),[screen,setScreen])
    return (<>
        {/* main container */}
        <main className="w-full h-screen m-auto flex justify-center items-center  animate-changeColor bg-400% bg-gradient">

            <RegisterScreenContext.Provider value={RegistrationScreenValue}>

           
            {/* registeration container */}
            <div className={` ${screen === 'sign-in'?" md:w-4/12 md:h-[85%] ":"sm:w-9/12 h-[97%] "}  transition-all duration-300 ease bg-background w-11/12 p-4  pb-2 m-auto  rounded-3xl md:h-[70%] overflow-hidden`}>
                <div>
                    <img className="w-36 m-6" src={"./images/logo.png"} />
                </div>
        {/* main screen */}

                <div className={`${screen ==='sign-in'?"overflow-y-hidden items-end":"overflow-y-auto"} relative   overflow-x-hidden flex h-full`}>
                 {/* intro section */}
                <section className={`${screen === 'main' ? " translate-x-0":" -translate-x-[-9999px] "}  absolute transition-transform duration-300 ease-in flex justify-center items-center flex-wrap  w-full`}>
                    ...
                    <div className="p-10  h-[25rem] w-full  md:w-5/12">
                ...
                {/* sign in screen */}
                <section className={`${screen === 'sign-in' ? "translate-x-0":"translate-x-[9999px] w-0 invisible"}  transition-transform duration-300   flex justify-center items-center   `}>
                    <div className="h-[30rem]  flex justify-center items-end m-auto">
                    <SignInComponent/>
                ...
                <section className={`${screen === 'register' ? "translate-x-0":"-translate-x-[-9999px] w-0 h-0 invisible"} flex pb-2  absolute  transition-transform duration-300 `}>
