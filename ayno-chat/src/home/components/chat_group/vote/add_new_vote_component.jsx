...
export default function AddNewVote(){
    ...
    const [error,setError] = useState({quiestion:null,choice:null,isEmpty:null})
            ...
            <div onClick={(event)=>{
                ...
                <div className="w-[70%] rounded-md h-[80%] bg-slate-700 overflow-y-auto p-3">
                    ...
                    <InputTextComponent onChange={(event)=>{
                        setError((pre)=>({...pre,quiestion:null}))

                    }} placeHolder="add your vote question"/>
                    <InputErrorComponent show={error.quiestion!==null?true:false} title="Please enter the vote question"/>
                    ...
                    <Plus onClick={()=>{
                        ...
                        if(pollText !== ""){
                            setPollChoices(prev=>[...prev,pollText])
                            setPollText("")
                    ...
                    <span className="text-slate-300 text-sm">Vote poll choices</span>
                        <>
                            {pollChoices.length !==0 ? <div className="flex flex-col w-[70%] h-auto">
                                {pollChoices.map((data,index)=> <div className="flex justify-between h-[2.5rem] w-full p-1"><p className=" block overflow-hidden text-ellipsis text-slate-200" key={data+"s"}> {data}</p><div
                                
                                onClick={(event)=>{
                                    let a = event.target.parentElement.previousSibling.innerText
                                setPollChoices(pollChoices.filter(dataFromArray=> dataFromArray !== a))
