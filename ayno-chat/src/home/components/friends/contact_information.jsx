...
function ContactInformation(props) {
  ...
  useEffect(() => {
    socket.on('online-users', (users) => {
      users.find(data => data.id === data.id) !== undefined ? setIsUserOnline(true) : setIsUserOnline(false)
    })
  ...
  return (
    <>
      {width <= 868 ? (
        // mobile version
        <div
          className={`${isMobile ? "opacity-1  visible" : "opacity-0   translate-x-[999px]"
            } overflow-x-hidden     transition-all absolute flex justify-end  right-0 bg-theme w-full h-full ease-in duration-100`}
        >
          {/* main container */}
          <div
            className={`${isMobile ? "translate-x-0" : "translate-x-[999px] "
          ...
          >
            <div className="  flex flex-col justify-center items-center w-full">
              ...
              <div className="relative w-20">
                ...
                <div className={`${isUserOnline?"scale-100":"scale-0"} transition-transform duration-100 ease-out w-4 h-4 bg-emerald-500 absolute bottom-0 rounded-full right-2 border-gray-800 border-2`}>{isUserOnline}</div>
              </div>
                    ...
                    {groupMembers.map((data, index) =>
                      <div key={data.name} className="p-1 m-1">
                        <img
                          ...
                          }} className="cursor-pointer rounded-md hover:scale-150 transition-transform duration-100 object-contain" src={data.profileImagePath} /></div>)}
                  </div>
                  <div className={`${selectedGroupMember === -1 ? "h-0 opacity-0" : "h-[99%]"} transition-all duration-300 ease-out overflow-hidden`}>
                    ...
                    <div className="flex">
                      ...
                      <div className="flex ml-1 flex-col justify-end items-start  ">
                        <h1 className="text-slate-200 ml-2 p-2 text-sm">{selectedGroupMemberData.name}</h1>
                        <h1 className="text-slate-200 ml-2 p-1 text-sm">{selectedGroupMemberData._id}</h1>
                      </div>
                    </div>
                    <div className=" w-10/12">

                      <div onClick={() => { setContact({ ...selectedGroupMemberData, type: "contact" }) }} className="ml-4 pl-2 items-center justify-center flex pr-2 p-1 rounded-md cursor-pointer mt-2 text-slate-100 bg-teal-700"><MessageSquare className="mr-1" /> <span>Message {selectedGroupMemberData.name}</span></div>
                    </div>
                  </div>
                </div>
              ) : (
                    ...
                    <div onMouseLeave={() => {
                      ...
                      <div className={`${selectedGroupMember === -1 ? "h-0 opacity-0" : "h-[99%]"} transition-all duration-300 ease-out overflow-hidden`}>
                        ...
                        <div className="flex">
                          ...
                          <div>
                            <h1 className="text-slate-200">{selectedGroupMemberData.name}</h1>
                            <h1 className="text-slate-200">{selectedGroupMemberData._id}</h1>
                          </div>
                        </div>
