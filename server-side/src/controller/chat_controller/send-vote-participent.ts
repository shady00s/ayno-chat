...
export default function sendVoteParticipent(req:Request,res:Response){
    ...
    groups_model.countDocuments({conversation_id:conversation_id,"messages.votingData.voteParticepents":{particepentChoice: particepentChoice, prticipentId:user_id }}).then((val)=>{
        console.log(val);
        if(val === 0){
